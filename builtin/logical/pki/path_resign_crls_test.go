...
func TestResignCrls_ForbidSigningOtherIssuerCRL(t *testing.T) {
	...
	resp, err = CBWrite(b, s, "issuer/default/resign-crls", map[string]any{
		"crl_number":  "2",
		"next_update": "1h",
		"format":      "pem",
		"crls":        []string{pem1},
...
func TestResignCrls_NormalCrl(t *testing.T) {
	...
	resp, err := CBWrite(b1, s1, "issuer/default/resign-crls", map[string]any{
		"crl_number":  "2",
		"next_update": "1h",
		"format":      "pem",
...
func TestResignCrls_ConflictingExpiry(t *testing.T) {
	...
	resp, err = CBWrite(b2, s2, "revoke", map[string]any{
		"certificate": cert1,
	})
...
func TestSignRevocationList(t *testing.T) {
	...
	resp, err = client.Logical().Write("pki/issuer/default/sign-revocation-list", map[string]any{
		"crl_number":  "1",
		"next_update": "12h",
		"format":      "pem",
		"revoked_certs": []map[string]any{
			{
...
func TestSignRevocationList_NoRevokedCerts(t *testing.T) {
	...
	resp, err = CBWrite(b, s, "issuer/default/sign-revocation-list", map[string]any{
		"crl_number":  "10000",
		"next_update": "12h",
		"format":      "pem",
	})
...
func setupResignCrlMounts(t *testing.T, b1 *backend, s1 logical.Storage, b2 *backend, s2 logical.Storage) (*x509.Certificate, string, string, string, string) {
	...
	resp, err := CBWrite(b1, s1, "root/generate/exported", map[string]any{
		"common_name": "test.com",
	})
	requireSuccessNonNilResponse(t, resp, err)
	...
	resp, err = CBWrite(b2, s2, "config/ca", map[string]any{
	...
	resp, err = CBWrite(b1, s1, "roles/test", map[string]any{
	...
	resp, err = CBWrite(b2, s2, "roles/test", map[string]any{
		"allowed_domains":  "test.com",
		"allow_subdomains": "true",
		"max_ttl":          "1h",
	})
	...
	resp, err = CBWrite(b1, s1, "issue/test", map[string]any{
	...
	resp, err = CBWrite(b1, s1, "revoke", map[string]any{"serial_number": serial1})
	requireSuccessNonNilResponse(t, resp, err, "error revoking cert from backend 2")

	// Issue and revoke a cert in backend 2
	resp, err = CBWrite(b2, s2, "issue/test", map[string]any{
		"common_name": "test1.test.com",
	})
	...
	resp, err = CBWrite(b2, s2, "revoke", map[string]any{"serial_number": serial2})
	requireSuccessNonNilResponse(t, resp, err, "error revoking cert from backend 2")
...
func requireExtensionOid(t *testing.T, identifier asn1.ObjectIdentifier, extensions []pkix.Extension, msgAndArgs ...any) {
	t.Helper()
