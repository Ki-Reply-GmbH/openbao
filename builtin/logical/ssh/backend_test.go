...
func TestProperAuthing(t *testing.T) {
	...
	_, err = client.Logical().WriteWithContext(ctx, "ssh/config/ca", map[string]any{
	...
	_, err = client.Logical().WriteWithContext(ctx, "ssh/roles/test-ca", map[string]any{
		"key_type":                "ca",
		"allow_user_certificates": true,
		"allowed_users":           "*",
	})
	...
	_, err = client.Logical().WriteWithContext(ctx, "ssh/issue/test-ca", map[string]any{
		"valid_principals": "toor",
	})
	if err != nil {
		t.Fatal(err)
	...
	_, err = client.Logical().WriteWithContext(ctx, "ssh/roles/test-ca-empty", map[string]any{
		"key_type":                "ca",
		"allow_host_certificates": true,
	...
	_, err = client.Logical().WriteWithContext(ctx, "ssh/issue/test-ca-empty", map[string]any{
		"cert_type": "host",
	...
	_, err = client.Logical().WriteWithContext(ctx, "ssh/roles/test-otp", map[string]any{
		"key_type":     "otp",
		"default_user": "toor",
	...
	resp, err := client.Logical().WriteWithContext(ctx, "ssh/creds/test-otp", map[string]any{
		"username": "toor",
		"ip":       "127.0.0.1",
	})
	if err != nil || resp == nil {
...
func submitCAIssuerStep(issuerName string, parameters map[string]any) logicaltest.TestStep {
	path := "issuers/import"
	if issuerName != "" {
		path += "/" + issuerName
	}
...
func updateIssuersConfigStep(parameters map[string]any) logicaltest.TestStep {
	return logicaltest.TestStep{
		Operation: logical.UpdateOperation,
		Path:      "config/issuers",
		Data:      parameters,
