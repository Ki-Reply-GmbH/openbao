...
func (lm *LockManager) RestorePolicy(ctx context.Context, storage logical.Storage, name, backup string, force bool) error {
	...
	if keyData.ArchivedKeys != nil {
		...
		if err != nil {
			return fmt.Errorf(fmt.Sprintf("failed to restore archived keys for key %q: %w", name), err)
		}
	}
	...
	if err != nil {
		return fmt.Errorf(fmt.Sprintf("failed to restore the policy %q: %w", name), err)
	}
...
func (lm *LockManager) DeletePolicy(ctx context.Context, storage logical.Storage, name string) error {
	...
	if err != nil {
		return fmt.Errorf(fmt.Sprintf("error deleting key %q: %w", name), err)
	}

	err = storage.Delete(ctx, "archive/"+name)
	if err != nil {
		return fmt.Errorf(fmt.Sprintf("error deleting key %q archive: %w", name), err)
	}
