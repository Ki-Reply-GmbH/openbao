...
func TestSysMount_put(t *testing.T) {
	...
	resp := testHttpPut(t, token, addr+"/v1/sys/mounts/foo", map[string]any{
		"type":        "kv",
		"description": "foo",
	})
	testResponseStatus(t, resp, 204)
...
func TestSysRemountSpacesFrom(t *testing.T) {
	...
	resp := testHttpPost(t, token, addr+"/v1/sys/mounts/foo%20bar", map[string]any{
		"type":        "kv",
		"description": "foo",
	})
	testResponseStatus(t, resp, 204)
...
func TestSysTuneMount_Options(t *testing.T) {
	...
	resp = testHttpPost(t, token, addr+"/v1/sys/mounts/foo/tune", map[string]any{
		"options": map[string]string{
			"test": "true",
		},
	})
	...
	expected := map[string]any{
		...
		"data": map[string]any{
			...
			"force_no_cache":    false,
			"options":           map[string]any{"test": "true"},
		},
		"description":       "foo",
		"default_lease_ttl": json.Number("2764800"),
		"max_lease_ttl":     json.Number("2764800"),
...
func TestSysTuneMount_passthroughRequestHeaders(t *testing.T) {
	...
	expected := map[string]any{
		...
		"data": map[string]any{
			...
			"options":                     map[string]any{"version": "1"},
			"force_no_cache":              false,
			"passthrough_request_headers": []any{"X-Vault-Foo"},
		},
		"description":                 "key/value secret storage",
		"default_lease_ttl":           json.Number("2764800"),
		"max_lease_ttl":               json.Number("2764800"),
...
func TestSysTuneMount_allowedManagedKeys(t *testing.T) {
	...
	expected := map[string]any{
		...
		"data": map[string]any{
			...
			"options":              map[string]any{"version": "1"},
			"force_no_cache":       false,
			"allowed_managed_keys": []any{"test_key"},
		},
		"description":          "key/value secret storage",
		"default_lease_ttl":    json.Number("2764800"),
		"max_lease_ttl":        json.Number("2764800"),
