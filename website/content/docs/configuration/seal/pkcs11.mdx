...
- A version of libc (likely `glibc` or `musl` with `gcompat`; OpenBao as
  released upstream is linked against Glibc but works on Musl with `gcompat`).
- When *not* using the PKCS#11 KMS plugin, a HSM-enabled build of OpenBao with
  PKCS#11 support compiled in via cgo.
...
- `slot` `(string: <optional>)`: The slot number to use, specified as a string
  (e.g., a decimal `"2305843009213693953"` or a hex `"0x2000000000000001"`). May
  also be specified by the `BAO_HSM_SLOT` environment variable.

- `serial` `(string: <optional>)`. The token serial number to use. May also be
  specified by the `BAO_HSM_SERIAL` environment variable.

- `token_label` `(string: <optional>)`: The slot token label to
