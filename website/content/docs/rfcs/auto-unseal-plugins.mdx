...
in the HSM distribution of OpenBao due to requiring CGO.

Furthermore, requests for (usually along with a Pull Request ready for review)
have been received to add additional vendor-specific mechanisms:
...
The outstanding, non-integrated providers are mostly regional, niche or upcoming
...
2. An OpenBao instance usually only uses one (up to two with [Seal
   Migration](../concepts/seal.mdx)) seal mechanisms at a time. This number
   may increase with [Emergency Seals](./emergency-seal.mdx) or [Namespace
   Sealing](./namespace-sealing.mdx), but it is unlikely for an instance to make
...
presented problem by pluginizing Auto Unseal mechanisms using the same plugin
mechanism that is already used for secrets, auth & database engines.

### User-facing description

OpenBao Auto Seal Plugins will be available as standalone
binaries (and OCI images) just like current secrets/auth/database
plugins are. The source code of these plugins will live in
...
mentioned, OCI-based plugin distribution will also be supported.

A generic "kms" plugin type is chosen over a more specific "seal" plugin type to
leave room for this new plugin type to service future KMS-related features such
as [External Keys](./external-keys.mdx) in addition to seals.
...
on unformal community observation, it appears that many instances are operated
