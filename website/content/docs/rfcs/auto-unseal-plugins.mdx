...
Furthermore, requests to add additional vendor-specific mechanisms have been
received have been received (usually along with a Pull Request ready for
review):
...
3. As outlined in [OpenBao's original Plugin
   Proposal](https://github.com/orgs/openbao/discussions/64), it is preferred to
   constrain builtin integrations with 3rd party components to easily testable,
   OSI-licensed components while keeping others external and encouraging broader
   community involvement to maintain externalized integrations.
