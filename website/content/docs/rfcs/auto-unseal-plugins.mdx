...
The technical implementation of this RFC will be split into two parts, 1) making
...
have been inherited into this repository from the original fork point.
...
2. Drop any unnecessary components of the protocol that are not needed for
   OpenBao's case. Appearently, the inherited definitions are used only
   by HashiCorp Boundary. Thus, some fields and features can be omitted."

3. Modify the plugin SDK to replace the singleton `Wrapper` instance with 
   a factory, enabling the on-demand creation of `Wrapper` instances.
   This mirrors the SDKs for existing secrets, auth & database plugins.
   It ensures compatibility with future OpenBao features requiring 
   multiple `Wrapper` instances of the same type, such as Per-Namespace Sealing
   with multiple concurrent Auto Seals.
...
4. Add `main` functions to each module in `wrappers/` to serve their respective
   ...
   *(Likely, this item should be updated considering recent comments in #1319.)*
...
- Ensuring KMS plugins are not registered into storage upon unseal, unlike 
  other plugins types.
...
on informal community observation many instances are operated
without external secrets, auth & database plugins. Apparently, the built-ins are
sufficient for most use cases, yet there is a large variety of choices when it comes to
...
affect other processes or cause the OpenBao instance to crash.
