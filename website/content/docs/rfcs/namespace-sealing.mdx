...
Introduce a per-namespace sealing mechanism to extend OpenBao's tenant separation capabilities. Expanding the Barrier + Keyring functionality, we want to create a mechanism to enable per-tenant encryption, which lets a tenant control their own seal keys in their namespace. OpenBao can thus provide a more robust and flexible multi-tenancy solution, addressing the shared encryption limitations and offering tenants more significant control over their data security and availability.
...
* Enhanced Namespace Lifecycle Management: Tenants would gain more control over the lifecycle of their namespace, including the ability to seal and unseal as needed.
...
Sealing an OpenBao Namespace pauses the Server's operations aimed at that namespace until a tenant operator decides to unseal it. Equal to global sealing, this means that a sealed namespace discards its in-memory namespace barrier keyring and namespace root key (which is required to decrypt the dedicated namespace barrier keyring), which physically prevents responding to operations when sealed.
...
Namespace sealing is optional. By default, namespaces are created without per-namespace sealing and they are part of their parent namespace's sealing/unsealing process. Namespace sealing only affects new namespaces created with this option. The root namespace and existing namespaces without a sealing config are not affected.
...
* OpenBao's storage layer for the namespace encounters an unrecoverable error.
...
3. For namespaces that are not configured with auto-unseal, OpenBao allows manual shamir unseal via the respective namespace specific api endpoint.
...
graph
    subgraph "Root (sealed)"
        ...
        SS_Root["Shamir-1..5 🔑<br/>Key shares / Unseal Keys"] --> |unseal root| RK["root key 🔑"]
        RK --> EK["encryption key 🔑"]
        ...
        subgraph "Namespace 1 (sealed)"
            NSRK1["namespace-1<br/>root-key 🔑"] --> |encrypts| NSEK1["namespace<br/>encryption key 🔑"]
        end
        SC_NS1 --> NSRK1
        SS_NS1["Shamir-1..5 🔑<br/>Key shares / Unseal Keys"] --> |unseal Namespace 1| NSRK1
        ...
        subgraph "Namespace 3 (sealed)"
            NSRK3["namespace-2<br/>root-key 🔑"] --> |encrypts| NSEK3["namespace<br/>encryption key 🔑"]
        end
        KMS --> |unseal Namespace 3| NSRK3
    end
```

At the level of the global sealing we use the root-key to encrypt the list of namespaces including their metadata. The seal configuration of a namespace is encrypted by the barrier key of the next sealable namespace upward in the hierarchy. With access to this data, each individual namespace can be decrypted by auto-unseal or a tenant with the namespace-root-key.
...
|------------ | -------------|
|`namespaces/<ns-uuid>/core/seals/<name>/shamir-config` OR <br/>`namespaces/<ns-uuid>/core/seals/<name>/recovery-config` | It stores the configuration of the seal itself.<br/><br/>Can be shamir-config or config for auto-unseal mechanism.<br/><br/>This storage entry is encrypted with the parent namespace's barrier, as it needs to be readable during unsealing of the namespace. |
|`namespaces/<ns-uuid>/core/seals/<name>/encrypted-root` | Used for storing the encrypted namespace-root key. |
|`namespaces/<ns-uuid>/core/seals/<name>/kek`<br/>(when applicable) | Storing a copy of the Shamir’s key for Shamir rekey operations and to restore raft snapshots.<br/><br/>This is a **barrier-encrypted** storage entry. |
|`namespaces/<ns-uuid>/core/seals/<name>/autounseal-config` | This allows for retrieval of the seal mechanisms and determining if configuration has changed since the last startup. <br /><br />This is a **barrier-encrypted** storage entry. |
|`namespaces/<ns-uuid>/core/seals/<name>/barrier-keyring` | We'll store a copy of the barrier keyring encrypted with that seal's current namespace-root key.<br/><br/>(Enables decryption of the current latest root key.) |
|`namespaces/<ns-uuid>/core/seals/<name>/current-root` | We'll store an encrypted copy of the seal's current namespace-root key<br/><br/>This is a **barrier-encrypted** storage entry. (Allows other nodes to rotate the barrier keyring keys.) |
|`namespaces/<ns-uuid>/core/seals/<name>/latest-root` | It stores the cluster’s latest namespace specific root key.<br/><br/>This is a **barrier-encrypted** storage entry. |
...
- seals (`map[string]map[string]interface{}: nil`) - A map containing maps of user-provided string to string key value pairs, containing sealing configuration. The outer key is the seal name.
...
Sealing a namespace also seals all of it's children recursively.
...
- `/sys/namespaces/:name/rekey`
- `/sys/namespaces/:name/rekey-recovery-key`
- `/sys/namespaces/:name/rotate`
- `/sys/namespaces/:name/rotate/config`
...
Per-Namespace encryption would increase tenant security, as there is no single key encrypting all of OpenBao's data.

Since the data in a sealable namespace is encrypted using its own chain of keys (barrier key ring, root key, unseal key) and not double encrypted with that of the parent. Key leakage would mean a higher risk of data compromise.
...
* How would namespace sealing works with [#1528](https://github.com/openbao/openbao/issues/1528)?
* How should the case when the active node goes offline be handled?
