...
Sealable namespaces are an optional feature. Namespaces created without a seal
configuration continue to work as before and are protected by the global OpenBao
seal.
...
- Even if the global root key is available, a sealed namespace's data remains
  inaccessible
- A server restart seals all namespaces; each must be unsealed independently
