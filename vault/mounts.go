...
func (m *mountable) mountInternal(ctx context.Context, entry *MountEntry, updateStorage bool) (err error) {
	m.lock.Lock()
	defer m.lock.Unlock()

	return m.mountInternalWithLock(ctx, entry, updateStorage)
}

func (m *mountable) mountInternalWithLock(ctx context.Context, entry *MountEntry, updateStorage bool) error {
	ns, err := namespace.FromContext(ctx)
	if err != nil {
		return err
	}
...
func (c *Core) splitNamespaceAndMountFromPath(currNs, path string) namespace.MountPathDetails {
	...
	ns, mountPath := c.namespaceStore.GetNamespaceByLongestPrefix(namespace.RootContext(context.Background()), fullPath)
