...
func TestNamespaceStore_DeleteNamespace(t *testing.T) {
	...
	// create namespace
	testNamespace := &NamespaceEntry{Namespace: &namespace.Namespace{Path: "test"}}
	err := s.SetNamespace(ctx, testNamespace)
	require.NoError(t, err)

	// retrieve namespace
	createdNS, err := s.GetNamespaceByPath(ctx, testNamespace.Namespace.Path)
	require.NoError(t, err)
	...
	parentNamespace := &NamespaceEntry{Namespace: &namespace.Namespace{Path: "parent/"}}
	childNamespace := &NamespaceEntry{Namespace: &namespace.Namespace{Path: "parent/child/"}}
	err = s.SetNamespace(ctx, parentNamespace)
	require.NoError(t, err)

	parentNS, err := s.GetNamespaceByPath(ctx, parentNamespace.Namespace.Path)
	require.NoError(t, err)

	parentCtx := namespace.ContextWithNamespace(ctx, parentNS.Namespace)
