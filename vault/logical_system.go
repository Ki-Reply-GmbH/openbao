...
func (b *SystemBackend) handleRemount(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	go func(migrationID string) {
		...
		if err != nil {
			...
			if err := b.Core.setMigrationStatus(migrationID, routing.MigrationFailureStatus); err != nil {
				logger.Error("Setting migration status failed", "error", err, "target_status", routing.MigrationFailureStatus)
			}
		}
	}(migrationID)
...
func (b *SystemBackend) moveMount(ns *namespace.Namespace, logger log.Logger, migrationID string, entry *routing.MountEntry, fromPathDetails, toPathDetails namespace.MountPathDetails) error {
	...
	if err := b.Core.setMigrationStatus(migrationID, routing.MigrationSuccessStatus); err != nil {
		return err
	}
