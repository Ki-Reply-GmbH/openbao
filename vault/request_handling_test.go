...
func TestRequestHandling_Wrapping(t *testing.T) {
	...
	err := core.mount(namespace.RootContext(context.TODO()), &MountEntry{
...
func TestRequestHandling_LoginWrapping(t *testing.T) {
	...
	if err := core.loadMounts(namespace.RootContext(context.TODO())); err != nil {
		t.Fatalf("err: %v", err)
	}
...
`,
	...
	tokenResp, err := core.HandleRequest(namespace.RootContext(context.TODO()), req)
	require.NoError(t, err)
