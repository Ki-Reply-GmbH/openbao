...
func TestPolicy_PaginationLimit(t *testing.T) {
	...
	for i := 1; i <= 100; i++ {
		_, err = client.KVv2("kv").Put(context.Background(), fmt.Sprintf("a/key-%v", i), map[string]any{
			"value": i,
		})
		require.NoError(t, err, "failed writing k/v key")

		_, err = client.KVv2("kv").Put(context.Background(), fmt.Sprintf("b/key-%v", i), map[string]any{
			"value": i,
		})
		require.NoError(t, err, "failed writing k/v key")

		_, err = client.KVv2("kv").Put(context.Background(), fmt.Sprintf("c/key-%v", i), map[string]any{
			"value": i,
		})
		require.NoError(t, err, "failed writing k/v key")

		_, err = client.KVv2("kv").Put(context.Background(), fmt.Sprintf("d/key-%v", i), map[string]any{
			"value": i,
		})
		require.NoError(t, err, "failed writing k/v key")
	}
