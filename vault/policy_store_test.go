...
func testPolicyStoreCRUDOneShot(t *testing.T, ps *PolicyStore, ns *namespace.Namespace) {
	...
	// Set should work
	ctx = namespace.ContextWithNamespace(context.Background(), ns)
	pol, _ := policy.ParseACLPolicy(ns, aclPolicy)
...
func testPolicyStoreACL(t *testing.T, ps *PolicyStore, ns *namespace.Namespace) {
	ctx := namespace.ContextWithNamespace(context.Background(), ns)
	pol, _ := policy.ParseACLPolicy(ns, policy_testing.ACLPolicy)
	err := ps.SetPolicy(ctx, pol, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	ctx = namespace.ContextWithNamespace(context.Background(), ns)
	pol, _ = policy.ParseACLPolicy(ns, policy_testing.ACLPolicy2)
	err = ps.SetPolicy(ctx, pol, nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	...
	policy_testing.TestLayeredACL(t, acl, ns)
}
