...
func (c *Core) mount(ctx context.Context, entry *routing.MountEntry) error {
	...
	// Mount internally
	if err := c.mountInternal(ctx, entry, routing.MountTableUpdateStorage); err != nil {
		return err
	}
...
func (c *Core) unmount(ctx context.Context, path string) error {
	...
	if err := c.unmountInternal(ctx, path, routing.MountTableUpdateStorage); err != nil {
		return err
	}
...
func (c *Core) createMigrationStatus(from, to namespace.MountPathDetails) (string, error) {
	...
	migrationInfo := routing.MountMigrationInfo{
		SourceMount:     from.Namespace.Path + from.MountPath,
		TargetMount:     to.Namespace.Path + to.MountPath,
		MigrationStatus: routing.MigrationInProgressStatus.String(),
	}
	c.mountMigrationTracker.Store(migrationID, migrationInfo)
	return migrationID, nil
}

func (c *Core) setMigrationStatus(migrationID string, migrationStatus routing.MountMigrationStatus) error {
	...
	migrationInfo := migrationInfoRaw.(routing.MountMigrationInfo)
	migrationInfo.MigrationStatus = migrationStatus.String()
	c.mountMigrationTracker.Store(migrationID, migrationInfo)
	return nil
}

func (c *Core) readMigrationStatus(migrationID string) *routing.MountMigrationInfo {
	migrationInfoRaw, ok := c.mountMigrationTracker.Load(migrationID)
	if !ok {
		return nil
	}
...
func (c *Core) mountEntryView(me *routing.MountEntry) (barrier.View, error) {
	...
	case routing.CredentialTableType:
		return NamespaceView(c.barrier, me.Namespace).SubView(path.Join(routing.CredentialBarrierPrefix, me.UUID) + "/"), nil
