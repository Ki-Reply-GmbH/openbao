...
func (c *Core) cleanupNamespaceMounts(ctx context.Context, mountTable *routing.MountTable, nsUUID string) error {
	for _, e := range mountTable.Entries {
		if e.Namespace.UUID == nsUUID {
			nsCtx := namespace.ContextWithNamespace(ctx, e.Namespace)

			routerPath := e.Path
			if mountTable.Type == routing.CredentialTableType {
				routerPath = routing.CredentialRoutePrefix + e.Path
			}
