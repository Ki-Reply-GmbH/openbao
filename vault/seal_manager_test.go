...
func TestSealManager(t *testing.T) {
	...
	sys := core.systemBackend
	rootCtx := namespace.RootContext(context.Background())
	ns, keyShares := testCreateNamespace(t, rootCtx, sys, "test", map[string]any{
	...
	for i := range 2 {
		...
		key, err := hex.DecodeString(keyShares["default"][i])
		require.NoError(t, err)
