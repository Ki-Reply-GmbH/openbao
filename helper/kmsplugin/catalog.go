...
type client struct {
	...
	clientproto plugin.ClientProtocol
}
...
func (c *Catalog) getClient(name string) (*client, bool, error) {
	...
	if c.pluginDirectory == "" {
		return nil, false, errors.New("plugin directory not configured")
	}
	...
	if err != nil {
		return nil, false, fmt.Errorf("invalid plugin sha256: %w", err)
	}
	...
	if err != nil {
		process.Kill()
		return nil, false, fmt.Errorf("start plugin client: %w", err)
	}
...
func (c *Catalog) checkFilePath(plugin *server.PluginConfig) error {
	...
	if plugin.Image == "" {
		...
		ok = parent == c.pluginDirectory
	} else {
		// Declarative, OCI-based plugin.
		ok = parent == filepath.Join(c.pluginDirectory, oci.PluginCacheDir, plugin.Slug(), plugin.SHA256Sum[:8])
	}
	if !ok {
...
func (c *client) close() {
	...
	case 1:
		// Last reference, so remove from lookup, close connections & kill the
		// process.
		c.refs--
